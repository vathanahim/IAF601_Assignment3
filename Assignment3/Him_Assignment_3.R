...
# considering median income and population does improve the linear regression model because it allows use to consider 
# other factors that may cause an increase in house_price_index. When we considered only coffee shops as variables, 
# we were able to see that there wasn't a strong linear relationship due to the p-value of dunkin_donuts being less
# than the 0.05 threshold and rather, starbucks was the only predictor variable that indicated an increase in
# house_price_index. However, after the introduction of these two additional variables,
# we're able to see other variables that may have strong influence in the increase of house_price_index.
...
#This can be seen based on the p-value of the respective variable. Dunking Donuts p-value went from 0.28 (before county information) to 0.58 (after county information). 
#Similary Startbucks p-value went from 0.0468 (before county information) to 0.11 (after county information). 
#This rise in p-value could suggests that Starbucks and Dunkin Donuts are not as significant as variables in comparison to county information in determing the house_price_index.
#Because house_price_index are more dependent on county information, this is what the summary regression tells us. 
#However, if we look at previous regression before county information is added, we can see that Starbucks and 
#Dunkin Donuts did play a role in influencing house_price_index because other confounding variables weren't analyzed.  
...
#Based on the data above Norfolk on average seems to have the most expensive house prices with an average value of 306.
#However, if we look at the summary regression from Q5-2, we can see that Suffolk county on average is 
#the most expensive and fastest growing due to its slope coefficient.
...
#I disagree that Starbucks is the bellwether of rise in house prices because once other variables 
# were considered in determining the rise in house_price_index, we were able to determine that startbucks became less "statistically" signicant as a predictor variable. Based on the regression analysis, 
# we were able to see that population, income, and county were more significant variables in determing the house_price_index. 
# As stated in the article "After a Starbucks opens in town, housing prices tend to rise, Harvard study finds", 
# the arrivals of starbucks may indicate an already trending of increase home value to confirm that gentrification is already happening. 
# Because we see a potential variable as a causation to the dependend variable doesn't mean it's the only variable that is affecting it; 
# we must consider other confounding variables as well.  
